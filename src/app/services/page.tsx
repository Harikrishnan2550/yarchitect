...
export default function ServicesPage() {
  ...
  useEffect(() => {
    const observerOptions = {
      root: null,
      rootMargin: '-25% 0px -35% 0px', // focused in the active scroll area
      threshold: 0.1
    };
  ...
  return (
    <main className="min-h-screen bg-cream relative overflow-x-clip text-forest selection:bg-sage selection:text-white" ref={containerRef}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-32 relative items-start">
          ...
          <div className="lg:col-span-4 sticky top-32 hidden lg:flex flex-col gap-6">
            ...
            <motion.h2 
              ...
              whileInView="visible" 
              variants={sideReveal} 
              className="font-serif text-[60px] font-light text-forest tracking-tighter leading-none"
            >
              Our <br/>
              <span className="italic text-sage">Legacy.</span>
            </motion.h2>
            ...
            <motion.p 
              ...
              whileInView="visible" 
              variants={sideReveal} 
              className="font-sans font-light text-[15px] leading-[1.8] text-forest/60 max-w-[280px] mb-4"
            >
              A holistic approach to construction and design, from planning to tactile interior details.
            </motion.p>

            {/* Scroll Spy Menu */}
            <div className="flex flex-col gap-5 border-l border-forest/15 pl-5 mt-4">
              {services.map((item, idx) => (
                <button
                  ...
                  onClick={() => {
                    const el = document.getElementById(`service-${idx}`);
                    if (el) {
                      const offset = 120; // offset spacing for sticky header
                ...
                >
                  <span className={`font-sans text-[10px] tracking-widest transition-colors duration-500 leading-none ${idx === activeIdx ? 'text-sage font-bold' : 'text-forest/30 group-hover:text-forest/50'}`}>
                    {item.num}
                  </span>
                  
                  <div className="relative overflow-hidden">
                    <span className={`font-serif text-lg block transition-all duration-500 ${idx === activeIdx ? 'text-forest translate-x-1 font-medium' : 'text-forest/30 group-hover:text-forest/50'}`}>
                      {item.title}
                    </span>
                    <motion.div 
                      className="absolute bottom-0 left-0 right-0 h-[1px] bg-sage origin-left"
                      initial={{ scaleX: 0 }}
                      animate={{ scaleX: idx === activeIdx ? 1 : 0 }}
          ...
          <div className="lg:col-span-8 flex flex-col gap-16 md:gap-32">
            {services.map((service, idx) => (
              <motion.div 
                ...
                variants={staggerContainer}
                className="flex flex-col group border-b border-forest/5 pb-16 md:pb-24 last:border-0 last:pb-0 scroll-mt-28"
              >
                <div className="flex flex-row items-baseline gap-4 md:gap-8 mb-6">
                  <motion.span 
                    variants={sideReveal} 
                    className={`font-serif text-[10vw] md:text-[80px] italic transition-colors duration-700 leading-none ${idx === activeIdx ? 'text-sage font-medium' : 'text-sage/20 group-hover:text-sage/40'}`}
                  >
                    {service.num}
                  </motion.span>
                  <motion.h3 
                    variants={sideReveal} 
                    className={`font-serif text-[8vw] md:text-[56px] tracking-tight font-light leading-none transition-colors duration-500 ${idx === activeIdx ? 'text-forest font-normal' : 'text-forest/70 group-hover:text-forest'}`}
                  >
                    {service.title}
                  </motion.h3>
                </div>
                
                <div className="md:pl-[120px] flex flex-col gap-6">
                  <motion.p 
                    variants={sideReveal} 
                    className="font-sans font-light text-[15px] md:text-[18px] leading-[1.8] text-forest/70 max-w-2xl"
                  >
                    {service.desc}
                  </motion.p>
                  <motion.div 
                  ...
                  >
                    {service.focus.map((item, i) => (
                      <span 
                        key={i} 
                        className={`px-4 py-1.5 rounded-full border transition-all duration-700 font-sans text-[8px] md:text-[11px] tracking-[0.2em] uppercase ${idx === activeIdx ? 'border-sage/40 bg-[#7A9C7E]/5 text-forest font-medium shadow-sm' : 'border-forest/10 bg-white/40 text-forest/70'}`}
                      >
                        {item}
                      </span>
                    ))}
