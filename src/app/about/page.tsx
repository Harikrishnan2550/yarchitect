    ...
    return (
      <main className="min-h-screen bg-[#FDFCFB] relative overflow-hidden text-[#0F2517]" ref={containerRef}>
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-[8%] relative z-10">
          ...
          <section id="directors" className="py-16 md:py-32 border-t border-[#0F2517]/10 mt-16 md:mt-24 relative scroll-mt-24">
            <AbstractGeometry className="top-10 left-10 w-[300px] h-[300px] rotate-45" />
            <HalftoneFade className="bottom-0 right-0 w-[200px] h-[400px]" />
            ...
            <motion.div 
              ...
              className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-24 relative z-10 items-start"
            >
              {/* Left Column - Image with Architectural Frame */}
              <div className="col-span-1 lg:col-span-5 flex flex-col gap-6">
                <motion.div 
                  variants={safeImageReveal}
                  className="w-full aspect-[3/4] overflow-hidden relative bg-[#0F2517]/5 rounded-2xl md:rounded-3xl shadow-[0_30px_70px_rgba(15,37,23,0.06)] border border-white"
                >
                  <img 
                    src="/shefin.png" 
                    alt="Ar. Shefin Yoosaf T" 
                    className="w-full h-full object-cover grayscale-[15%] contrast-[105%]"
                  />
                  {/* Passepartout line */}
                  <div className="absolute inset-4 border border-white/30 rounded-[16px] md:rounded-[24px] pointer-events-none" />
                </motion.div>
                
                {/* Meta details below image */}
                <motion.div variants={fadeUp} className="flex justify-between items-center px-2">
                  <div className="flex flex-col">
                    <span className="font-serif text-[15px] italic text-[#0F2517]">Ar. Shefin Yoosaf T</span>
                    <span className="font-sans text-[9px] tracking-wider text-[#0F2517]/40 uppercase">Principal Director</span>
                  </div>
                  <span className="font-sans text-[8px] tracking-[0.2em] text-[#7A9C7E] uppercase font-bold border border-[#7A9C7E]/20 px-3 py-1 rounded-full">
                    Active Reg.
                  </span>
                </motion.div>
              </div>

              {/* Right Column - Detailed Profile & Blueprint Specs */}
              <div className="col-span-1 lg:col-span-7 flex flex-col justify-center">
                <motion.span variants={fadeUp} className="font-sans text-[10px] tracking-[0.3em] text-[#7A9C7E] font-bold uppercase mb-4 flex items-center gap-4">
                  01 <span className="w-6 h-[1px] bg-[#7A9C7E]" /> Leadership
                </motion.span>
                
                <motion.h2 variants={fadeUp} className="font-serif text-[11vw] md:text-[56px] leading-[1] text-[#0F2517] mb-2 tracking-tight">
                  Ar. Shefin Yoosaf T
                </motion.h2>
                
                <motion.p variants={fadeUp} className="font-sans text-xs md:text-sm tracking-[0.15em] uppercase font-medium text-[#7A9C7E] mb-8">
                  Principal Architect | Town Planner (A), Y Architects
                </motion.p>
                
                <motion.p variants={fadeUp} className="font-sans font-light text-[14px] md:text-[16px] leading-[1.9] text-[#0F2517]/70 mb-10">
                  Ar. Shefin Yoosaf T is an Architect and Town Planner leading Y Architects, a multidisciplinary practice established in 2015 in Malappuram. With a B.Arch from GEC Thrissur and an M.Plan from CET Trivandrum, his work focuses on contextual, functional, and climate-responsive design. He is registered with the Council of Architecture, accredited as a Town Planner, and is a member of the Indian Institute of Architects.
                </motion.p>

                {/* Blueprint Credentials Table/Grid */}
                <motion.div variants={staggerContainer} className="flex flex-col border-t border-[#0F2517]/10 pt-2 w-full">
                  {[
                    { label: "B.Arch", value: "GEC Thrissur" },
                    { label: "M.Plan", value: "CET Trivandrum" },
                    { label: "COA Reg. No.", value: "CA/2015/73233" },
                    { label: "Town Planner Reg. No.", value: "E-2050/08/14714/KKD/3/2019/TPA" },
                    { label: "IIA Membership No.", value: "23379" }
                  ].map((item, idx) => (
                    <motion.div 
                      key={idx} 
                      variants={fadeUp}
                      className="group flex flex-col md:flex-row justify-between py-4 border-b border-[#0F2517]/10 hover:bg-[#7A9C7E]/5 px-3 transition-colors duration-300"
                    >
                      <span className="font-sans text-[11px] md:text-[12px] tracking-[0.15em] text-[#0F2517]/50 uppercase font-medium group-hover:text-[#0F2517] transition-colors duration-300">
                        {item.label}
                      </span>
                      <span className="font-serif text-[15px] md:text-[16px] text-[#0F2517] font-light mt-1 md:mt-0">
                        {item.value}
                      </span>
                    </motion.div>
                  ))}
          ...
          <section className="flex flex-col gap-12 md:gap-32 border-t border-[#0F2517]/10 pt-12 md:pt-24 relative">
            ...
            <div className="flex flex-col lg:flex-row items-start lg:items-center gap-6 lg:gap-20 relative z-10">
              ...
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="w-full lg:w-1/2 flex flex-col justify-center">
                <motion.span variants={fadeUp} className="font-sans text-[10px] tracking-[0.3em] text-[#7A9C7E] font-bold uppercase mb-3 md:mb-6 flex items-center gap-4">
                  02 <span className="w-6 h-[1px] bg-[#7A9C7E]" /> Philosophy
                </motion.span>
            ...
            <div className="flex flex-col lg:flex-row-reverse items-start lg:items-center gap-6 lg:gap-20 relative z-10">
              ...
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="w-full lg:w-7/12 flex flex-col justify-center lg:ml-auto">
                <motion.span variants={fadeUp} className="font-sans text-[10px] tracking-[0.3em] text-[#7A9C7E] font-bold uppercase mb-3 md:mb-6 flex items-center gap-4">
                  03 <span className="w-6 h-[1px] bg-[#7A9C7E]" /> Approach
                </motion.span>
